...

{
  ...
  # environment.variables.XDG_DATA_DIRS = lib.mkForce "/home/mihai/.nix-profile/share:/run/current-system/sw/share";
  ...
  boot.kernelModules = [ "tcp_bbr" "kvm-intel" ]; # FIX: Network Congestion Control (Helps with packet jitter)
  boot.kernel.sysctl = {
  ...
  hardware.nvidia = {
    ...
    # PRIME CONFIGURATION (Hybrid Graphics - Intel + NVIDIA)
    prime = {
      offload = {
        enable = true;
      ...
      # Bus IDs derived from lspci output
      # NVIDIA GTX 1650 Ti Mobile: 01:00.0 -> PCI:1:0:0
      # Intel UHD Graphics: 00:02.0 -> PCI:0:2:0
